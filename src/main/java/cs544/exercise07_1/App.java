...
public class App {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            String origin = "USA";
            @SuppressWarnings("unchecked")
            Query query = session.createQuery("from Flight f where f.airplane.capacity > :capacity "
                    + " AND f.origin.country = :origin");
            query.setParameter("capacity", capacity);
            query.setParameter("origin", origin);
            List<Flight> flights = query.list();

            System.out.println();
            System.out.println("Flights leaving USA with capacity > 500");
            System.out.println("Flight:  Departs:     "
                    + "                  Arrives:                           Capacity:      Origin:");
            for (Flight flight : flights) {
                System.out.printf("%-7s  %-12s %7s %8s  %-12s %7s %8s %8s %14s\n",
                        flight.getFlightnr(), flight.getOrigin().getCity(),
                        flight.getDepartureDate(), flight.getDepartureTime(),
                        flight.getDestination().getCity(), flight
                                .getArrivalDate(), flight.getArrivalTime(),
                        flight.getAirplane().getCapacity(), flight.getOrigin().getCountry());
            }
            System.out.println();
            tx.commit();
        } catch (HibernateException e) {
        ...
        try {
            ...
            String model = "A380";
            @SuppressWarnings("unchecked")
            Query query = session.createQuery("select distinct aln from Airline aln join aln.flights f "
                    +"where f.airplane.model = :model");
        ...
        try {
            ...
            String model = "747";
            String airline = "Star Alliance";
            // TODO update HQL
            @SuppressWarnings("unchecked")
            Query query = session.createQuery("select distinct f from Flight f where f.airplane.model = :model "
                    +"AND f.airline.name <> :airline");
            query.setParameter("model", model);
            ...
            for (Flight flight : flights) {
                System.out.printf("%-7s  %-12s %7s %8s  %-12s %7s %8s %8s %12s\n",
                        flight.getFlightnr(), flight.getOrigin().getCity(),
                        flight.getDepartureDate(), flight.getDepartureTime(),
                        flight.getDestination().getCity(), flight
                                .getArrivalDate(), flight.getArrivalTime(), flight.getAirplane().getModel(), flight.getAirline().getName());
            }
            System.out.println();
            tx.commit();
        } catch (HibernateException e) {
        ...
        try {
            ...
            Date departureDate = dateFormatter.parse("2009-08-07");

            SimpleDateFormat timeFormatter = new SimpleDateFormat("HH:mm:ss");
            Date departureTime = timeFormatter.parse("12:00:00");
            // TODO update HQL
            @SuppressWarnings("unchecked")
    ...
    public static void fillDataBase() {
        ...
        try {
            ...
            Airport fra = new Airport("FRA", "Frankfurt International Airport",
                    "Frankfurt", "Germany");
            Airport dtw = new Airport("DTW", "Detroit City", "Detroit", "USA");
